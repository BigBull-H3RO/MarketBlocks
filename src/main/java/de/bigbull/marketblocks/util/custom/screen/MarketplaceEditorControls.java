...
import de.bigbull.marketblocks.network.packets.marketplace.MarketplaceCreatePagePacket;
import de.bigbull.marketblocks.network.packets.marketplace.MarketplaceDeleteOfferPacket;
...
/**
 * Builds and wires edit-mode controls for {@link MarketplaceScreen}.
 */
public final class MarketplaceEditorControls {
    ...
    public void build(Context context, Callbacks callbacks) {
        ...
        MarketplacePage page = context.pages().get(Math.min(context.selectedPage(), context.pages().size() - 1));
    ...
    private void addOfferListControls(Context context, Callbacks callbacks, MarketplacePage page) {
        if (context.selectedOfferId() != null) {
            ...
            callbacks.addWidget(new IconButton(deleteX, deleteY, 20, 20, context.buttonSprites(), context.deleteIcon(),
                    ...
                    Component.translatable("gui.marketblocks.marketplace.delete_offer"), () -> false));
    ...
    public interface Callbacks {
        ...
        void openOfferLimitsEditor(MarketplaceOffer offer);

        void openOfferPricingEditor(MarketplaceOffer offer);
