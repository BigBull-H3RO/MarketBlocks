...
public class ServerShopMenu extends AbstractContainerMenu {
    ...
    public void setSelectedOffer(UUID offerId) {
        ...
        if (!ServerShopManager.get().isOfferOnPage(offerId, selectedPage())) {
            setCurrentTradingOffer(null);
            return;
        }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (index == RESULT_SLOT) {
            if (isEditMode) {
                ...
                int maxByLimit = ServerShopManager.get().getMaximumPurchasableNow(currentTradingOffer.id(), serverPlayer.getUUID());
    ...
    public ServerShopOfferViewState currentOfferViewState() { return currentTradingOffer == null ? ServerShopOfferViewState.empty() : ServerShopClientState.offerViewState(currentTradingOffer.id()); }
    ...
    private int getCurrentMaxPurchasable() {
        ...
        if (inventory.player instanceof net.minecraft.server.level.ServerPlayer serverPlayer) {
            return ServerShopManager.get().getMaximumPurchasableNow(currentTradingOffer.id(), serverPlayer.getUUID());
        }
    ...
    private class ServerShopResultSlot extends Slot {
        public ServerShopResultSlot(Container container, int index, int x, int y) {
        ...
        public void onTake(Player player, ItemStack stack) {
            if (!isEditMode && currentTradingOffer != null && player instanceof net.minecraft.server.level.ServerPlayer serverPlayer) {
                PaymentMatch paymentMatch = resolvePaymentMatch();
                if (paymentMatch == null || ServerShopManager.get().getMaximumPurchasableNow(currentTradingOffer.id(), serverPlayer.getUUID()) <= 0
                        || !ServerShopManager.get().processPurchaseTransactionSlotBased(serverPlayer, currentTradingOffer.id(), 1)) {
