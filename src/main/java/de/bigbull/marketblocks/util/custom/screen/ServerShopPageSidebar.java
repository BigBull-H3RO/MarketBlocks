...
/**
 * Handles ServerShop page sidebar buttons, truncation and delayed hover tooltips.
 */
public final class ServerShopPageSidebar {
    ...
    public void buildButtons(Context context, Callbacks callbacks) {
        ...
        for (int i = 0; i < context.pages().size(); i++) {
            int pageIndex = i;
            ServerShopPage page = context.pages().get(i);
    ...
    private Component pageDisplayName(ServerShopPage page, int index) {
        if (page.name().isBlank()) {
            return Component.translatable("gui.marketblocks.server_shop.unnamed_page", index + 1);
        }
