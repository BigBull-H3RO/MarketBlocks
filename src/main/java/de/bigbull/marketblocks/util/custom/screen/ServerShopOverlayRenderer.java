...
/**
 * Renders offer list/status overlays for ServerShopScreen without owning screen state.
 */
public final class ServerShopOverlayRenderer {
    ...
    private void renderEmptyState(GuiGraphics guiGraphics, Context context) {
        if (!context.hasPages()) {
            Component noPagesText = Component.translatable("gui.marketblocks.server_shop.no_pages");
    ...
    private void renderOfferRows(GuiGraphics guiGraphics, Context context) {
        ...
        for (int i = context.scrollOffset(); i < end; i++) {
            ServerShopOffer offer = context.visibleOffers().get(i);
    ...
    private void renderSelectedOfferStatus(GuiGraphics guiGraphics, Context context) {
        ...
        ServerShopOffer offer = context.offerLookup().apply(selectedOfferId);
        if (offer == null) {
            return;
        }

        ServerShopOfferViewState viewState = ServerShopClientState.offerViewState(selectedOfferId);
    ...
    private void renderAvailabilityIcon(GuiGraphics guiGraphics, Context context, ServerShopOfferViewState viewState, int textX, int textY) {
    ...
    private int renderPriceAndDailyStatus(GuiGraphics guiGraphics, Context context, ServerShopOfferViewState viewState, int textX, int textY) {
        Component priceText = Component.translatable(
                "gui.marketblocks.server_shop.status.price_short",
        ...
        if (!context.suppressInteractions() && isPointWithinStatusLine(context, textX, textY, priceText)) {
            guiGraphics.renderTooltip(context.font(), Component.translatable("gui.marketblocks.server_shop.tooltip.price_multiplier"), context.mouseX(), context.mouseY());
        }
        ...
        if (viewState.remainingDailyPurchases().isPresent()) {
            ...
            Component dailyText = Component.translatable("gui.marketblocks.server_shop.status.daily_short", remainingDaily);
            ...
            if (!context.suppressInteractions() && isPointWithinStatusLine(context, compactX, textY, dailyText)) {
                Component tooltip = remainingDaily == 0
                        ? Component.translatable("gui.marketblocks.server_shop.tooltip.remaining_daily_empty")
                        : Component.translatable("gui.marketblocks.server_shop.tooltip.remaining_daily");
    ...
    private int renderStockStatus(GuiGraphics guiGraphics, Context context, ServerShopOfferViewState viewState, int textX, int textY) {
        if (viewState.remainingStock().isPresent()) {
            ...
            Component stockText = Component.translatable("gui.marketblocks.server_shop.status.stock_short", remainingStock);
            ...
            if (!context.suppressInteractions() && isPointWithinStatusLine(context, textX, textY, stockText)) {
                Component tooltip = remainingStock == 0
                        ? Component.translatable("gui.marketblocks.server_shop.tooltip.remaining_stock_empty")
                        : Component.translatable("gui.marketblocks.server_shop.tooltip.remaining_stock");
                guiGraphics.renderTooltip(context.font(), tooltip, context.mouseX(), context.mouseY());
            }
            return textY + context.statusLineHeight();
        }
    ...
    private void renderRestockStatus(GuiGraphics guiGraphics, Context context, ServerShopOfferViewState viewState, int textX, int textY) {
        ...
        if (displayRestockSeconds.isPresent()) {
            ...
            String restockValue = restockSeconds > 0
                    ? ServerShopRuntimeMath.formatSecondsAsTimer(restockSeconds)
                    : "0:00";
            Component restockText = Component.translatable("gui.marketblocks.server_shop.status.restock_short", restockValue);
            ...
            if (!context.suppressInteractions() && isPointWithinStatusLine(context, textX, textY, restockText)) {
                Component tooltip = restockSeconds > 0
                        ? Component.translatable("gui.marketblocks.server_shop.tooltip.restock_in")
                        : Component.translatable("gui.marketblocks.server_shop.tooltip.restock_ready");
                guiGraphics.renderTooltip(context.font(), tooltip, context.mouseX(), context.mouseY());
            }
        }
    }

    private void renderOfferRow(GuiGraphics graphics, Context context, ServerShopOffer offer, int x, int y, boolean isSelected) {
        ServerShopOfferViewState viewState = ServerShopClientState.offerViewState(offer.id());
    ...
    private void renderHoveredRowTooltip(GuiGraphics graphics, Context context, int listStartX, int listStartY) {
        ...
        ServerShopOffer offer = context.visibleOffers().get(offerIndex);
        ServerShopOfferViewState viewState = ServerShopClientState.offerViewState(offer.id());
        ItemStack[] payments = context.paymentNormalizer().apply(offer.effectivePayments());
    ...
    private void renderRowTooltips(GuiGraphics graphics, Context context, ItemStack p1, ItemStack p2, ItemStack result,
                                   ServerShopOfferViewState viewState,
    ...
    private Component buildUnavailableOfferTooltip(Context context, ServerShopOfferViewState viewState) {
        if (viewState.remainingDailyPurchases().isPresent() && viewState.remainingDailyPurchases().get() <= 0) {
            return Component.translatable("gui.marketblocks.server_shop.tooltip.unavailable_daily");
        }
        if (viewState.remainingStock().isPresent() && viewState.remainingStock().get() <= 0) {
            if (context.displayRestockSecondsResolver().apply(viewState).isPresent()) {
                return Component.translatable("gui.marketblocks.server_shop.tooltip.unavailable_restock");
            }
            return Component.translatable("gui.marketblocks.server_shop.tooltip.unavailable_stock");
        }
        return Component.translatable("gui.marketblocks.server_shop.tooltip.unavailable_generic");
    }
    ...
    public record Context(
            ...
            int scrollOffset,
            int maxVisibleRows,
            List<ServerShopOffer> visibleOffers,
            UUID selectedOfferId,
            WidgetSprites buttonSprites,
            ResourceLocation arrowIcon,
            Function<UUID, ServerShopOffer> offerLookup,
            Function<List<ItemStack>, ItemStack[]> paymentNormalizer,
            Function<ServerShopOfferViewState, ResourceLocation> unavailableIconResolver,
            Function<ServerShopOfferViewState, Optional<Integer>> displayRestockSecondsResolver
    ) {
        int listStartX() {
